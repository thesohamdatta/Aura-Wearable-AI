"""
Tools for answering questions about the Aura/Friend product.
"""
...
@tool
def get_aura_product_info_tool(query: str) -> str:
    """
    Get information about the Aura/Friend product to answer questions about features, functionality, setup, or purchasing.

    Use this tool when the user asks about:
    - How Aura/Friend works
    - What features the app/device has
    - How to set up or use Aura
    ...
    Args:
        query: The specific question about Aura/Friend product (e.g., "How does the device connect to my phone?", "What is the battery life?")

    Returns:
        Product documentation content from GitHub that can help answer the question

    Example:
        query="How do I update the firmware on my Aura device?"
    ...
    context_str = 'Aura/Friend Product Documentation:\n\n'
    for section, content in context.items():
        context_str += f'## {section}\n\n{content}\n\n'

    context_str += (
        '\n\nUse this documentation to answer questions about the Aura/Friend product, its features, setup, and usage.'
    )
